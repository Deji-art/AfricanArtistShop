...
app.get('/api/top-artists',(req,res)=>{const rows=db.prepare(`SELECT ar.id,ar.name,ar.specialty,ar.bio,ar.image_url,ar.artist_type,COUNT(a.id) artwork_count FROM artists ar LEFT JOIN artworks a ON a.artist_id=ar.id AND a.status='approved' WHERE ar.status='approved' GROUP BY ar.id ORDER BY artwork_count DESC, ar.name LIMIT 10`).all();res.json(rows)});
app.post('/api/training-requests',(req,res)=>{const {child_name,child_age,guardian_name,guardian_phone,email,location,training_type,notes}=req.body||{};if(!child_name||!child_age||!guardian_name||!guardian_phone||!email)return res.status(400).json({error:'Please complete the required training enquiry fields'});const info=db.prepare('INSERT INTO training_requests(child_name,child_age,guardian_name,guardian_phone,email,location,training_type,notes) VALUES(?,?,?,?,?,?,?,?)').run(child_name,Number(child_age),guardian_name,guardian_phone,email,location||'',training_type||'',notes||'');res.json({id:info.lastInsertRowid,status:'received'})});
